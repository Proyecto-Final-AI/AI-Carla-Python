...
def average_slope_intercept(image, lines):
    ...
    if lines is not None:
        for line in lines:
            ...
            if not y1 - y2 < 50 and y1 - y2 > -50:
                continue
            parameters = np.polyfit((x1, x2), (y1, y2), 1)
            parameters.astype(np.int64)
            slope = parameters[0]
...
def region_of_interest(image):
    polygons = np.array([[(140, 700), (1150, 700), (694, 380), (580, 380)]])
...
def process_img(image):
    ...
    lines = cv2.HoughLinesP(cropped_image, 2, np.pi/180, 100, np.array([]), minLineLength=20) ## Mirar esto minLineLength
