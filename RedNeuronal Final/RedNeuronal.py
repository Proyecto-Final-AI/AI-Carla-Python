...
class CarEnv:
    ...
    def getPoints(self, event, key):
        self.decisions[key] = event.distance
        v = self.vehicle.get_velocity()
        kmh = int(3.6 * math.sqrt(v.x**2 + v.y**2 + v.z**2))
        self.decisions[kmh] = kmh
        dection = Decisiones.Decision(self.decisions)
        self.points = dection.action
    ...
    def reset(self):
        ...
        self.obs_sensor_frontal.listen(lambda event: self.getPoints(event, "frontal"))
        self.obs_sensor_frontal_right.listen(lambda event: self.getPoints(event,"frontal_right"))
        self.obs_sensor_frontal_left.listen(lambda event: self.getPoints(event,"frontal_left"))
        
        self.obs_sensor_central_right.listen(lambda event: self.getPoints(event,"central_right"))
        self.obs_sensor_central_left.listen(lambda event: self.getPoints(event, "central_left"))
        
        self.obs_sensor_back.listen(lambda event: self.getPoints(event, "back"))
        self.obs_sensor_back_right.listen(lambda event: self.getPoints(event, "back_right"))
        self.obs_sensor_back_left.listen(lambda event: self.getPoints (event, "back_left"))
